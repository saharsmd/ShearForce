...
if __name__ == "__main__":
    ...
    executable = Path(*Path.cwd().parts[:-4],
                      "chaste_build2/projects/ShearForce/test",
                      "TestShearForceVelocityAlignment")
    ...
    outdir_stem = "Shear_Force_Velocity_Alignment_tension_output/"
    ...
    outdir_labels = ["nx", "dt", "P0", "F0", "F1", "KA", "KP", "K", "seed"]

    # Specify sets of model simulation parameters

    dt = 0.1  # timestep for vertex and ODE solver (I keep these the same)
    ...
    const_arg_dict = {
        ...
        # Initial mean values of variable
        "-init_erk": 0.0,
        ...
        "-flat_bottom": 1,    # (bool) Whether to initialize flat top
    ...
    # predicted period of oscillation)
    const_arg_dict["-bonus_time"] = np.round(100*calc_period(tau_e, const_arg_dict["-taul"]))
    # Specify a sampling interval for the data capture period
    const_arg_dict["-sampling_timestep_multiple"] = 10*int(0.1/dt)
    ...
    n_cpus = 6    # Number of simultaneous processes to run, i.e
